...
int OpenFile::ReadAt(char *into, int numBytes, int position)
{
    ...
    for (i = firstSector; i <= lastSector; i++)
        g_disk_driver->ReadSector(hdr->ByteToSector(i * g_cfg->SectorSize), &buf[(i - firstSector) * g_cfg->SectorSize]);
...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    if (!lastAligned && ((firstSector != lastSector) || firstAligned))
        ReadAt(&buf[(lastSector - firstSector) * g_cfg->SectorSize], g_cfg->SectorSize, lastSector * g_cfg->SectorSize);
    ...
    for (i = firstSector; i <= lastSector; i++)
        g_disk_driver->WriteSector(hdr->ByteToSector(i * g_cfg->SectorSize), &buf[(i - firstSector) * g_cfg->SectorSize]);
    return numBytes;
}
...
//----------------------------------------------------------------------
void OpenFile::SetName(char *newname)
{
    strcpy(name, newname);
}
