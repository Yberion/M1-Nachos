/*! \file filesys.cpp 
 ...
 //	   - A bitmap of free disk sectors (cf. bitmap.hpp)
...
/*! 	Create a file in the Nachos file system (similar to UNIX create).
 ...
 //              code as define in msgerror.hpp
 */
//----------------------------------------------------------------------
int FileSystem::Create(char *name, int initialSize)
{
    ...
    {
        g_open_file_table->createLock->Release();
        return ALREADY_IN_DIRECTORY; // file is already in directory
    }
    ...
    {
        g_open_file_table->createLock->Release();
        return OUT_OF_DISK; // no free block for file header
    }
    ...
    {
        g_open_file_table->createLock->Release();
        return OUT_OF_DISK; // no space on disk for data
    }
...
/*! 	Delete a file from the file system.  This requires:
 ...
 //	\return NO_ERROR if the file was deleted, else an error code
 //              as defined in msgerror.hpp
 */
//----------------------------------------------------------------------
int FileSystem::Remove(char *name)
{
    ...
    if (dirsector == -1)
    {
        return INEXIST_DIRECTORY_ERROR;
    }
    ...
    if (sector == -1)
        return INEXIST_FILE_ERROR; // file not found
...
void FileSystem::List()
{
    ...
    printf("Free Space : %d bytes ( %d %% )\n", bitmap.NumClear() * g_cfg->SectorSize,
            (int)((float)(bitmap.NumClear() * g_cfg->SectorSize) * 100 / (float)(NUM_SECTORS * g_cfg->SectorSize)));
...
/*! 	Create a directory in the Nachos file system (similar to UNIX create).
 ...
 //	\return NO_ERROR if everything goes ok, an error code otherwise
 //              (see msgerror.hpp)
 */
//----------------------------------------------------------------------
int FileSystem::Mkdir(char *dirname)
{
    ...
    if (parentdir.Find(name) >= 0)
        return ALREADY_IN_DIRECTORY; // Le sous-rep existe deja !
    ...
    if (hdr_sect < 0)
        return OUT_OF_DISK; // plus de place sur le disque
...
/*! 	Delete a directory from the file system.  This requires:
 ...
 //	       (NOT MODIFIED)
 //	\return NO_ERROR if the directory was deleted, else an error
 //              code (see msgerror.hpp)
 */
//----------------------------------------------------------------------
int FileSystem::Rmdir(char *dirname)
{
    ...
    if (thedirsect < 0)
        return INEXIST_DIRECTORY_ERROR; // directory not found
    ...
    if (!thedir.empty())
        return DIRECTORY_NOT_EMPTY; //directory is not empty
