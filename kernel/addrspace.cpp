...
/** 	Create an address space to run a user program.
 ...
 //
 //      Executables are in ELF (Executable and Linkable Format) (see elf32.hpp)
...
AddrSpace::AddrSpace(OpenFile *exec_file, Process *p, int *err)
{
    ...
    *err = 0;
    ...
    for (i = 0; i < elfHdr.e_shnum; i++)
    {
        ...
        printf("\t- Section %s : file offset 0x%x, size 0x%x, addr 0x%x, %s%s\n", section_name, (unsigned)section_table[i].sh_offset,
                (unsigned)section_table[i].sh_size, (unsigned)section_table[i].sh_addr, (section_table[i].sh_flags & SHF_WRITE) ? "R/W" : "R",
        ...
        for (unsigned int pgdisk = 0, virt_page = section_table[i].sh_addr / g_cfg->PageSize; pgdisk < divRoundUp(section_table[i].sh_size, g_cfg->PageSize);
                pgdisk++, virt_page++)
        {
            ...
            if (section_table[i].sh_type != SHT_NOBITS)
            {
                ...
                exec_file->ReadAt((char*)&(g_machine->mainMemory[translationTable->getPhysicalPage(virt_page) * g_cfg->PageSize]), g_cfg->PageSize,
...
#define STACK_BLANK_LEN 4 // in pages
    ...
    for (int i = stackBasePage; i < (stackBasePage + numPages); i++)
    {
        ...
        memset(&(g_machine->mainMemory[translationTable->getPhysicalPage(i) * g_cfg->PageSize]), 0x0, g_cfg->PageSize);
 ...
 */
//----------------------------------------------------------------------
static void SwapELFHeader(Elf32_Ehdr *ehdr)
{
 ...
 */
//----------------------------------------------------------------------
static void CheckELFHeader(Elf32_Ehdr *elfHdr, int *err)
{
    /* Make sure it is an ELF file by looking at its header (see elf32.hpp) */
    if (elfHdr->e_ident[EI_MAG0] != 0x7f || elfHdr->e_ident[EI_MAG1] != 'E' || elfHdr->e_ident[EI_MAG2] != 'L' || elfHdr->e_ident[EI_MAG3] != 'F')
    {
    ...
    if (elfHdr->e_ident[EI_CLASS] != ELFCLASS32 || elfHdr->e_ident[EI_VERSION] != EV_CURRENT)
    {
        printf("2\n");
        *err = EXEC_FILE_FORMAT_ERROR;
        return;
    ...
    if (elfHdr->e_machine != EM_MIPS || elfHdr->e_type != ET_EXEC)
    {
        *err = EXEC_FILE_FORMAT_ERROR;
        return;
    }
    ...
    /* Make sure ELF file internal structures are consistent with what
     we expect */
    if (elfHdr->e_ehsize != sizeof(Elf32_Ehdr) || elfHdr->e_shentsize != sizeof(Elf32_Shdr))
    {
        *err = EXEC_FILE_FORMAT_ERROR;
        return;
    }
