/*! \file disk.cpp 
 ...
 //	See disk.hpp for details about the behavior of disks (and
...
/*!	Returns how long it will take to position the disk head over the correct
 ...
 //
 //   	Disk seeks at one track per SEEK_TIME nanos (cf. stats.hpp)
...
int Disk::TimeToSeek(int newSector, int *rotation)
{
    ...
    int seek = abs(newTrack - oldTrack) * nano_to_cycles(SEEK_TIME, g_cfg->ProcessorFrequency);
    // how long will seek take?
    int over = (g_stats->getTotalTicks() + seek) % (nano_to_cycles(ROTATION_TIME, g_cfg->ProcessorFrequency));
