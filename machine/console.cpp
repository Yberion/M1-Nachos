 ...
 */
//----------------------------------------------------------------------
Console::Console(char *readFile, char *writeFile, VoidNoArgFunctionPtr readAvail, VoidNoArgFunctionPtr writeDone)
{
    if (readFile == NULL)
...
void Console::CheckCharAvail()
{
    ...
    if (intState)
        g_machine->interrupt->Schedule(ConsoleReadPoll, (int64_t)this, nano_to_cycles(CONSOLE_TIME, g_cfg->ProcessorFrequency), CONSOLE_READ_INT);
...
void Console::PutChar(char ch)
{
    ...
    putBusy = true;
    g_machine->interrupt->Schedule(ConsoleWriteDone, (int64_t)this, nano_to_cycles(CONSOLE_TIME, g_cfg->ProcessorFrequency), CONSOLE_WRITE_INT);
}
