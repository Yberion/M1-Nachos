...
#include "vm/pagefaultmanager.hpp"
 ...
 */
//----------------------------------------------------------------------
bool MMU::ReadMem(uint32_t virtAddr, int size, uint32_t *value, bool is_instruction)
{
    uint32_t data;
    ExceptionType exc;
    uint32_t physAddr;
 ...
 */
//----------------------------------------------------------------------
bool MMU::WriteMem(uint32_t addr, int size, uint32_t value)
{
    ExceptionType exc;
    ...
    switch (size)
    {
        ...
        case 2:
            *(unsigned short*)&g_machine->mainMemory[physicalAddress] = ShortToMachine((unsigned short)(value & 0xffff));
            break;

        case 4:
            *(unsigned int*)&g_machine->mainMemory[physicalAddress] = WordToMachine((unsigned int)value);
            break;
        default:
            ASSERT(false)
            ;
