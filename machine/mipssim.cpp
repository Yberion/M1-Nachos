...
//      \param reg Register type according to the types defined in machine.hpp
 ...
 */
//----------------------------------------------------------------------
int Machine::OneInstruction(Instruction *instr)
{
    uint32_t raw; // binary representation of the instruction
    int nextLoadReg = 0;
    ...
    // Constant execution time for user instructions (see stats.hpp)
    ...
    if (DebugIsEnabled('m'))
    {
        ...
        if (instr->opCode == OP_BEQ || instr->opCode == OP_BGEZAL || instr->opCode == OP_BGEZ || instr->opCode == OP_BGTZ || instr->opCode == OP_BLEZ
                || instr->opCode == OP_BLTZAL || instr->opCode == OP_BLTZ || instr->opCode == OP_BNE || instr->opCode == OP_JAL || instr->opCode == OP_J
                || instr->opCode == OP_JALR || instr->opCode == OP_JR || instr->opCode == OP_BC1F || instr->opCode == OP_BC1T)
        {
            // In case of a branch, extra is not directly the offset
            printf(stri->string, IndexToAddr(instr->extra), TypeToReg(stri->args[1], instr), TypeToReg(stri->args[2], instr));
        }
        else
        {
            // Normal instruction
            printf(stri->string, TypeToReg(stri->args[0], instr), TypeToReg(stri->args[1], instr), TypeToReg(stri->args[2], instr));
        }
    ...
    switch (instr->opCode)
    {
        ...
        case OP_ADD:
            ...
            if (!((int_registers[(int)instr->rs] ^ int_registers[(int)instr->rt]) & SIGN_BIT) && ((int_registers[(int)instr->rs] ^ sum) & SIGN_BIT))
            {
        ...
        case OP_ADDI:
            ...
            if (!((int_registers[(int)instr->rs] ^ instr->extra) & SIGN_BIT) && ((instr->extra ^ sum) & SIGN_BIT))
            {
                RaiseException(OVERFLOW_EXCEPTION, 0);
                return 0;
            }
        ...
        case OP_ADDU:
            int_registers[(int)instr->rd] = int_registers[(int)instr->rs] + int_registers[(int)instr->rt];
            break;
        ...
        case OP_ANDI:
            int_registers[(int)instr->rt] = int_registers[(int)instr->rs] & (instr->extra & 0xffff);
            break;
        ...
        case OP_DIV:
            ...
            {
                int_registers[LO_REG] = int_registers[(int)instr->rs] / int_registers[(int)instr->rt];
                int_registers[HI_REG] = int_registers[(int)instr->rs] % int_registers[(int)instr->rt];
            }
            break;
        ...
        case OP_MULT:
            Mult(int_registers[(int)instr->rs], int_registers[(int)instr->rt], true, &int_registers[HI_REG], &int_registers[LO_REG]);
            break;

        case OP_MULTU:
            Mult(int_registers[(int)instr->rs], int_registers[(int)instr->rt], false, &int_registers[HI_REG], &int_registers[LO_REG]);
            break;
        ...
        case OP_OR:
            int_registers[(int)instr->rd] = int_registers[(int)instr->rs] | int_registers[(int)instr->rs];
            break;
        ...
        case OP_SB:
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 1, int_registers[(int)instr->rt]))
                return 0;
            break;

        case OP_SH:
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 2, int_registers[(int)instr->rt]))
                return 0;
            break;
        ...
        case OP_SLLV:
            int_registers[(int)instr->rd] = int_registers[(int)instr->rt] << (int_registers[(int)instr->rs] & 0x1f);
            break;
        ...
        case OP_SUB:
            ...
            if (((int_registers[(int)instr->rs] ^ int_registers[(int)instr->rt]) & SIGN_BIT) && ((int_registers[(int)instr->rs] ^ diff) & SIGN_BIT))
            {
                RaiseException(OVERFLOW_EXCEPTION, 0);
                return 0;
            }
        ...
        case OP_SW:
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 4, int_registers[(int)instr->rt]))
                return 0;
            break;
        ...
        case OP_SWL:
            ...
            switch (tmp & 0x3)
            {
                case 0:
                    ...
                    break;
                case 1:
                    value = (value & 0xff000000) | ((int_registers[(int)instr->rt] >> 8) & 0xffffff);
                    break;
                case 2:
                    value = (value & 0xffff0000) | ((int_registers[(int)instr->rt] >> 16) & 0xffff);
                    break;
                case 3:
                    value = (value & 0xffffff00) | ((int_registers[(int)instr->rt] >> 24) & 0xff);
                    break;
            }
        ...
        case OP_SWC1:
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 4, float_registers[(int)instr->ft]))
                return 0;
            break;
        ...
        case OP_SDC1:
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra), 4, float_registers[(int)instr->ft]))
                return 0;
            if (!mmu->WriteMem((unsigned)(int_registers[(int)instr->rs] + instr->extra + 4), 4, float_registers[(int)instr->ft + 1]))
                return 0;
            break;
        ...
        case OP_CVT_S_W:
            *((float*)&float_registers[(int)instr->fd]) = (float)float_registers[(int)instr->fs];
            break;
        case OP_CVT_W_S:
            float_registers[(int)instr->fd] = (int)(*((float*)&float_registers[(int)instr->fs]));
            break;
        case OP_CVT_D_W:
        ...
        case OP_UNIMP:
        {
            ...
            printf(str->string, TypeToReg(str->args[0], instr), TypeToReg(str->args[1], instr), TypeToReg(str->args[2], instr));
            printf("\n");
        }
...
void Instruction::Decode()
{
    ...
    { /* Floating point instructions */
        if (rs == 0x10)
        {
            opCode = cop1STable[value & 0x3f];
        }
        else if (rs == 0x11)
        {
            opCode = cop1DTable[value & 0x3f];
        }
        else if (rs == 0x08 && rt == 0x00)
        {
            opCode = OP_BC1F;
        }
        else if (rs == 0x08 && rt == 0x01)
        {
            opCode = OP_BC1T;
        }
        else if (rs == 0x00)
